...
namespace Argus.Windows.Forms.Controls
{
    ...
    public class BindableListView : ListView
    {
        ...
        public BindableListView()
        {
            ...
            this.ColumnClick += this.BindableListView_ColumnClick;
        }
        ...
        public object DataSource
        {
            ...
            set
            {
                ...
                if (value is DataTable)
                {
                    ...
                    {
                        var items = new string[((DataTable) value).Columns.Count];
                        int loopTo = ((DataTable) value).Columns.Count - 1;
                ...
                {
                    int loopTo1 = ((IDataReader) value).FieldCount - 1;
                    // *****************************************************************
                    ...
                    {
                        var items = new string[((IDataReader) value).FieldCount + 1];
                        int loopTo2 = ((IDataReader) value).FieldCount - 1;
                ...
                if (this.CleanupColumnHeadings)
                {
                    foreach (ColumnHeader c in this.Columns)
                    {
                        c.Text = this.CamelCaseToTitleCase(c.Text);
                        c.Name = this.CamelCaseToTitleCase(c.Name);
                    }
                }

                // Resize to the size of the column heading if property is true.
                if (this.ResizeOnDataSourceChange)
                {
        ...
        {
            if (this.DefaultSortingOnColumnClick)
            {
                this.BeginUpdate();
        ...
        {
            this.ApplyItemColors(this.RowItemColor, this.RowAlternatingItemColor);
        }
        ...
        private void ApplyFilter()
        {
            ...
            {
                if (_filteredItems[i].Text.Contains(this.Filter))
                {
