...
namespace Argus.IO.Compression
{
    ...
    public class ZipEntry
    {
        ...
        {
            int signature = Argus.IO.Compression.Shared.ReadSignature(s);
            ...
            ze._FileNameInArchive = Argus.IO.Compression.Shared.StringFromBuffer(block, 0, block.Length);
            ...
            ze._LastModified = Argus.IO.Compression.Shared.PackedToDateTime(ze._LastModDateTime);

            // actually get the compressed size and CRC if necessary
            if ((ze._BitField & 0x0008) == 0x0008)
            {
                long posn = s.Position;
                long SizeOfDataRead = Argus.IO.Compression.Shared.FindSignature(s, ZipConstants.ZipEntryDataDescriptorSignature);
        ...
        internal static ZipEntry Create(String filename, string DirectoryPathInArchive)
        {
            ...
            if (entry._LastModified.IsDaylightSavingTime())
            {
                ...
                entry._LastModDateTime = Argus.IO.Compression.Shared.DateTimeToPacked(AdjustedTime);
            }
            else
                entry._LastModDateTime = Argus.IO.Compression.Shared.DateTimeToPacked(entry._LastModified);
