...
namespace Argus.Data.SemanticLibrary
{
    public class KeywordAnalyzer
    {
        ...
        {
            var analysis = new KeywordAnalysis {Content = content};
            int wordCount = 0;
                                                                                      ...
                                                                                      SortedDictionary<decimal, string> termsG)
        {
            ...
            // *       = termFwg is Dictionary<string, Dictionary<string, decimal>>
            var termFwg = new Dictionary<string, Dictionary<string, decimal>>();
                ...
                foreach (var pair in termsG)
                {
                    ...
                    if (g != w)
                    {
                        ...
                        foreach (var paragraph in paragraphs)
                        {
                            ...
                            {
                                if (this.TermsCoOccur(sentence, w, g))
                                {
                                    sentCountWG++;
                                }
                            }
        ...
        {
            if (this.TermInSentence(sentence, w) && this.TermInSentence(sentence, g))
            {
                return true;
            }
                                                                    ...
                                                                    SortedDictionary<decimal, string> termsG, ref Dictionary<string, int> termNw, ref int termTotal)
        {
            ...
            var termPg = new Dictionary<string, decimal>();
                ...
                foreach (var paragraph in paragraphs)
                {
                    foreach (var sentence in paragraph.Sentences)
                    {
                        ...
                        if (found)
                        {
                            ...
                            {
                                termsInSentencesForTerm += sentence.Words.Count - 2; //all three word terms
                            }

                            if (sentence.Words.Count > 1)
                            {
                                termsInSentencesForTerm += sentence.Words.Count - 1; //all two word terms
                            }
                        }
                    }
                }
        ...
        {
            var probabilityTerms = new SortedDictionary<decimal, string>();
            var termsG = new SortedDictionary<decimal, string>();
            ...
            if (toptenCount > counts.Count)
            {
                toptenCount = counts.Count; //just in case there are so few
            }

            var ptkey = new decimal[probabilityTerms.Count];
