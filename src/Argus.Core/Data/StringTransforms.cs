...
namespace Argus.Data
{
    ...
    public class StringTransforms
    {
        ...
        {
            string[] lines = text.Split(lineTerminator.ToCharArray());
        ...
        /// <returns></returns>
        /// <remarks></remarks>
        public static string FormatDeliminatedString(string text, string deliminator)
        {
            ...
            foreach (string line in lines)
            {
                ...
                foreach (string field in fields)
                {
                    outputSb.AppendFormat("{0,-" + (fieldLength[counter] + 4).ToString() + "}", field.Trim());
                    counter = counter + 1;
                }
