...
namespace Argus.Extensions
{
    public static class DataTableExtensions
    {
        ...
        {
            string[] columnList = new string[dt.Columns.Count - 1];
        ...
        /// <param name="distinctList">Whether or not all of the values should be returned.  E.g. If an ID of 1 is in the list 10 times, it can either
        /// be returned once indicating it's a duplicate or it can be returned all 10 times.</param>
        public static List<object> SelectDuplicates(this DataTable dt, string columnName, bool distinctList)
        {
            ...
            {
                if (dt.Select(string.Format("{0} = {1}", columnName, dt.Rows[x][columnName])).Length > 1)
                {
                    if (distinctList)
                    {
        ...
        {
            var columnIndex = dt.Columns.IndexOf(columnName);
        ...
        public static void MergeXml(this DataTable dt, string filePathOne, string filePathTwo, string destinationPath)
        {
            ...
            {
                Exception mergeEx = new Exception("The XML files could not be read.  " + ex.Message, ex);
                throw mergeEx;
            }
            ...
            {
                Exception mergeEx = new Exception("The XML files could not be merged.  " + ex.Message, ex);
                throw mergeEx;
            }
            ...
            {
                Exception mergeEx = new Exception("The XML files could not be written.  " + ex.Message, ex);
                throw mergeEx;
            }
        ...
        {
            Argus.Data.CreateDelimitedFile ff = new Argus.Data.CreateDelimitedFile(exportHeader, delimiter);
        ...
        {
            var columnIndex = dr.Table.Columns.IndexOf(columnName);
        ...
        public static string ToString(this DataRow dr)
        {
            ...
            for (int x = 0; x <= dr.Table.Columns.Count; x++)
            {
                sb.AppendFormat("{0}{1}", dr[x].ToString().Replace("\t", "\\t"), "\t");
            }
